...
def resize_image(img: Image.Image, cfg: CONFIGCLASS) -> Image.Image:
    interp = sample_discrete(cfg.rz_interp)
    return TF.resize(img, cfg.loadSize, interpolation=rz_dict[interp])

def augment_image(img: Image.Image, cfg: CONFIGCLASS) -> Image.Image:
...
def binary_dataset(root: str, cfg: CONFIGCLASS) -> datasets.ImageFolder:
    identity_transform = transforms.Lambda(lambda img: img)

    if cfg.isTrain or cfg.aug_resize:
        rz_func = transforms.Lambda(lambda img: resize_image(img, cfg))
    else:
    ...
    return datasets.ImageFolder(
        ...
        transforms.Compose(
            [
                rz_func,
                transforms.Lambda(lambda img: augment_image(img, cfg)),
                crop_func,
                flip_func,
                transforms.ToTensor(),
